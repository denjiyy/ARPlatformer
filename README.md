...
## Quick Start (2 минути)

1. Отворете проекта в Unity и заредете основната AR сцена с `ARPlatformerRuntime`.
2. Build/Run на iOS устройство (разрешете Camera достъп).
3. Сканирайте помещението (под + горни повърхности на мебели), после натиснете **Finish Scan**.
4. Покажете marker-а към камерата, за да се появи героят.
5. Движете се с joystick, съберете монетите и стигнете флага.
...
- **Unity Editor**
  - Tested: проектът е валидиран в текущата версия, записана в `ProjectSettings/ProjectVersion.txt`.
- **Vuforia Engine**
  - Required: версия, съвместима с избраната Unity версия и поддържаща Runtime Image Target + Area Target Capture.
- **iOS**
  - Tested devices: iPhone 17 Pro Max, iPhone 14 Pro.
  - Required capabilities: ARKit-capable устройство + camera permission.
- **Render Pipeline / Input**
  - URP е предпочитан; има shader fallback логика.
  - Unity Input System е необходим за UI/touch контролите.
...
- Качеството на gameplay силно зависи от scan coverage, осветление и текстура на повърхностите.
- При много sparse/шумна геометрия fallback позициите може да са визуално неточни (макар и функционални).
- Няма автоматизирани playmode/regression тестове в текущата версия.
- Няма интегрирана production telemetry/crash analytics инфраструктура.
- Physics layer auto-correct защитата работи глобално за сесията и трябва да се преоцени при multi-scene разширение.
